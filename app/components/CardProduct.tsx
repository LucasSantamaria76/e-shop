...
*/
const CardProduct = ({ name, slug, images, price }: IProduct) => {
	return (
		<div className='flex flex-col justify-between rounded-lg bg-white overflow-hidden mx-auto max-w-xs md:max-w-60 shadow-md shadow-black/30 border border-black/30'>
			<Link href='#' />
			...
			<div className='px-6 py-4 bg-gray-100'>
				<p className=''>{name}</p>
				<p className='text-xs'>{formatPrice(price || 0)}</p>
			</div>
		</div>
	);
};
